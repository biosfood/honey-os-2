...
void handleGetServiceIdSyscall(Thread *call) {
    call->returnValue = ((Service *)call->service)->id;
}

void handleGetServiceSyscall(Thread *call) {
    uint32_t i = 0;
...
void handleGetFunctionSyscall(Thread *call) {
    uint32_t i = 0;
    Service *callService = call->service;
...
void handleCreateFunctionSyscall(Thread *call) {
    char *name = retrieveString(call->parameters[0]);
    if (!name) {
        return;
    }
...
void handleRequestSyscall(Thread *call) {
    Service *service = call->service;
    Service *providerService = listGet(services, call->parameters[0]);
    if (!providerService)
        return;
...
void handleLookupSymbolSyscall(Thread *call) {
...
void handleStackContainsSyscall(Thread *call) {
    Thread *currentCall = call;
    while (currentCall) {
