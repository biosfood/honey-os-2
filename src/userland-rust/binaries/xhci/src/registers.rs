...
impl XhciRegisters {
    pub fn map(phys_base: u32, size: u32) -> Result<Self, String> {
        ...
        println!("Opening MMIO path: {}", path);
        ...
        let file = match OpenOptions::new().read(true).write(true).open(&path) {
            ...
            Err(e) => {
                println!("Could not open {} ({}), trying /kernel/mem...", path, e);
                is_raw = true;
                OpenOptions::new()
                    .read(true)
                    .write(true)
        ...
        println!("Calling mmap (fd: {}, offset: 0x{:x}, size: 0x{:x})...", fd, mmap_offset, size);
        ...
        println!("mmap returned virt: {:p}", virt);
