...
use crate::pci::{unmask_irq, PciDevice};
...
impl XhciController {
    pub fn new(pci_dev: PciDevice) -> Result<Self, String> {
        println!("[xhci] Mapping MMIO BAR0: 0x{:x} (size: 0x{:x})", pci_dev.bar0, pci_dev.bar0_size);
        ...
        println!(
            "[xhci] Controller CapLength: 0x{:x}, Version: {}.{}, MaxSlots: {}, MaxPorts: {}",
            cap_len,
            (hci_ver >> 8) & 0xFF,
            hci_ver & 0xFF,
            max_slots,
        ...
        // Open IRQ file from /dev/pic/<irq>
        let irq = pci_dev.irq;
        println!("[xhci] Controller assigned IRQ: {}", irq);
        unmask_irq(irq);

        let irq_path = format!("/dev/pic/{}", irq);
        let irq_file = OpenOptions::new().read(true).open(&irq_path).ok();
        if irq_file.is_some() {
            println!("[xhci] Subscribed to IRQ notification on {}", irq_path);
        } else {
            println!("[xhci] Warning: Could not open {}, will fallback to polling", irq_path);
        }

        // 1. Reset host controller
        println!("[xhci] Resetting host controller...");
        // Clear Run/Stop (RS)
        ...
        println!("[xhci] Controller reset successful!");
        ...
        if sp_count > 0 {
            println!("[xhci] Allocating {} scratchpad buffers...", sp_count);
        ...
        println!("[xhci] Initializing Command Ring...");
        ...
        println!("[xhci] Initializing Event Ring and ERST...");
        ...
        println!("[xhci] Starting Host Controller...");
        ...
        println!("[xhci] Host controller running! (USBSTS: 0x{:x})", sts);
        ...
        F: FnMut(&Trb) -> bool,
    {
        for _ in 0..2000 {
            ...
            // Block waiting for IRQ on /dev/pic/<irq>
            if let Some(ref mut f) = self.irq_file {
                let mut buf = [0u8; 1];
    ...
    pub fn enumerate_devices(&mut self) -> Vec<DiscoveredUsbDevice> {
        ...
        for port in 1..=self.port_count {
            ...
            println!("[xhci] Device detected on Port {}! PORTSC: 0x{:08x}", port, portsc);
            ...
            if !port_enabled {
                println!("[xhci] Port {} reset failed!", port);
                continue;
            }
            ...
            println!("[xhci] Port {} enabled at {}", port, speed_name);

            // Enable slot
            let slot_id = match self.enable_slot() {
                Ok(id) => id,
                Err(e) => {
                    println!("[xhci] Failed to enable slot for port {}: {}", port, e);
                    continue;
                }
            };
            println!("[xhci] Assigned Slot ID {} to Port {}", slot_id, port);

            // Allocate Device Context (1 page) and put physical address in DCBAA[slot_id]
            let dev_ctx = match DmaBuffer::allocate(1) {
                Ok(b) => b,
                Err(e) => {
                    println!("[xhci] Failed to allocate device context: {}", e);
                    continue;
                }
            };
            ...
            let ep0_ring = match TrbRing::new(256) {
                Ok(r) => r,
                Err(e) => {
                    println!("[xhci] Failed to create EP0 ring: {}", e);
                    continue;
                }
            };
            ...
            let input_ctx = match allocate_input_context(port, speed, initial_max_packet, ep0_ring_phys) {
                Ok(b) => b,
                Err(e) => {
                    println!("[xhci] Failed to allocate input context: {}", e);
                    continue;
                }
            };
            ...
            if let Err(e) = self.address_device(slot_id, true) {
                println!("[xhci] AddressDevice(BSR=1) failed: {}", e);
                continue;
            }
            if let Err(e) = self.address_device(slot_id, false) {
                println!("[xhci] AddressDevice(BSR=0) failed: {}", e);
                continue;
            }
            ...
            let dev_desc = match self.get_device_descriptor(slot_id) {
                Ok(d) => d,
                Err(e) => {
                    println!("[xhci] Failed to read full device descriptor: {}", e);
                    continue;
                }
            };
            ...
            let config_bytes = match self.get_configuration_descriptor(slot_id) {
                Ok(b) => b,
                Err(e) => {
                    println!("[xhci] Failed to read configuration descriptor: {}", e);
                    continue;
                }
            };
            ...
            let (config, interfaces) = match parse_configuration(&config_bytes) {
                Some(res) => res,
                None => {
                    println!("[xhci] Failed to parse configuration descriptor!");
                    continue;
                }
            };
            ...
            if let Err(e) = self.configure_endpoint_cmd(slot_id) {
                println!("[xhci] ConfigureEndpoint command failed: {}", e);
            }

            // Set configuration
            if let Err(e) = self.set_configuration(slot_id, config.b_configuration_value) {
                println!("[xhci] SetConfiguration failed: {}", e);
            }
