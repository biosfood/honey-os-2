...
/// A physically contiguous DMA buffer allocated in userland memory.
...
impl DmaBuffer {
    /// Allocate `pages` (each 4KB) of physically contiguous, zeroed DMA memory.
    pub fn allocate(pages: usize) -> Result<Self, String> {
        ...
        // Query physical address of first page via /proc/self/pagemap
